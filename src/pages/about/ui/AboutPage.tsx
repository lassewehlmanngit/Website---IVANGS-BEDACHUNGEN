...
export const AboutPage: React.FC<{ lang: SupportedLang }> = ({ lang }) => {
  ...
  const { data, isLoading } = useAboutPageData(lang);
      ...
      <div className="animate-fade-in">
        ...
        <div className="bg-slate-900 text-white py-16 md:py-20 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 text-center relative z-10">
            <span className="text-primary font-bold uppercase tracking-wider text-xs sm:text-sm mb-3 md:mb-4 block" data-tina-field={data?.aboutPage?.hero && tinaField(data.aboutPage.hero, 'eyebrow')}>
              {hero.eyebrow || 'Das Unternehmen'}
            </span>
            <h1 className="text-h1 font-bold mb-4 md:mb-6" data-tina-field={data?.aboutPage?.hero && tinaField(data.aboutPage.hero, 'title')}>
              {hero.title || 'Seit 1996: Ein Familienbetrieb, der hoch hinaus will.'}
            </h1>
            <p className="text-slate-300 text-base md:text-lg max-w-2xl mx-auto leading-relaxed" data-tina-field={data?.aboutPage?.hero && tinaField(data.aboutPage.hero, 'description')}>
        ...
        <section className="py-16 md:py-20 bg-white">
          <div className="container mx-auto px-4 grid md:grid-cols-2 gap-12 md:gap-16 items-center">
             <div>
                <h2 className="text-h2 font-bold text-slate-900 mb-4 md:mb-6" data-tina-field={data?.aboutPage?.story && tinaField(data.aboutPage.story, 'title')}>
                  {story.title || 'Qualität ist bei uns Familiensache.'}
                </h2>
                <p className="text-slate-600 text-lg leading-relaxed mb-6" data-tina-field={data?.aboutPage?.story && tinaField(data.aboutPage.story, 'text1')}>
                ...
                <p className="text-slate-600 text-lg leading-relaxed mb-8" data-tina-field={data?.aboutPage?.story && tinaField(data.aboutPage.story, 'text2')}>
                ...
                <div className="flex flex-col gap-4">
                   {(about.values || [{ text: 'Inhabergeführt seit über 25 Jahren' }, { text: 'Ausbildungsbetrieb (derzeit 5 Azubis)' }, { text: 'Modernster Maschinenpark & eigener Kran' }]).map((value: any, i: number) => (
                     <div key={i} className="flex items-center gap-3" data-tina-field={value && tinaField(value, 'text')}>
             ...
             <div className="relative">
                ...
                <OptimizedImage 
                  ...
                  data-tina-field={data?.aboutPage?.story && tinaField(data.aboutPage.story, 'image')}
                />
             </div>
          </div>
        </section>
        ...
        <section className="py-16 md:py-20 bg-primary/5">
           <div className="container mx-auto px-4 text-center max-w-4xl">
              ...
              <h2 className="text-h2 font-bold text-slate-900 mb-3 md:mb-4" data-tina-field={data?.aboutPage?.equipment && tinaField(data.aboutPage.equipment, 'title')}>
                {equipment.title || 'Wir warten nicht auf Material. Wir haben es.'}
              </h2>
              <p className="text-slate-600 text-lg leading-relaxed" data-tina-field={data?.aboutPage?.equipment && tinaField(data.aboutPage.equipment, 'description')}>
        ...
        <section className="py-16 md:py-20 bg-white">
          <div className="container mx-auto px-4">
            <div className="mb-12 md:mb-16 text-center">
               <h2 className="text-h2 font-bold text-slate-900" data-tina-field={data?.aboutPage?.teamSection && tinaField(data.aboutPage.teamSection, 'title')}>
                 {teamSection.title || 'Unser Team'}
               </h2>
               <p className="text-slate-600 mt-4 max-w-2xl mx-auto" data-tina-field={data?.aboutPage?.teamSection && tinaField(data.aboutPage.teamSection, 'description')}>
        ...
        <section className="py-16 md:py-20 bg-slate-900 text-white">
          <div className="container mx-auto px-4 text-center">
             <h2 className="text-h2 font-bold mb-4 md:mb-6" data-tina-field={data?.aboutPage?.cta && tinaField(data.aboutPage.cta, 'title')}>
               {cta.title || 'Lernen Sie uns kennen'}
             </h2>
             <p className="text-slate-300 max-w-2xl mx-auto mb-8" data-tina-field={data?.aboutPage?.cta && tinaField(data.aboutPage.cta, 'description')}>
             ...
             <a href={`mailto:${cta.email || 'bedachungen@ivangs.de'}`} className="inline-flex items-center gap-2 bg-primary text-white px-8 py-4 rounded-sm font-bold hover:bg-primary/90 transition-colors shadow-lg" data-tina-field={data?.aboutPage?.cta && tinaField(data.aboutPage.cta, 'buttonText')}>
