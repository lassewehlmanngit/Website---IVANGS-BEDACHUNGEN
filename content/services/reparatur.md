---
title: Reparatur & Service
subtitle: Schnelle Hilfe & Wartung
shortDescription: Sturmschaden? Undichtes Fenster? Lose Ziegel? Wir kümmern uns sofort um den Werterhalt.
intro: Ein kleiner Schaden kann schnell teuer werden. Wasser sucht sich seinen Weg. Wir reparieren schnell, unkompliziert und nachhaltig – damit aus Kleinigkeiten keine Großbaustellen werden.
heroImage: /images/services/reparatur-service.jpg
icon: ShieldCheck
expertTip: Warten Sie nicht auf den Sturm. Ein Wartungsvertrag ist wie der TÜV fürs Auto – und oft günstiger als ein Wasserschaden.
uiText:
  introHeader: Wir lassen Sie nicht im Regen stehen.
  contactButtonText: Schaden melden
  careerCtaTitle: Karriere bei Ivangs
  careerCtaDescription: Werde Teil unseres 28-köpfigen Teams. Wir suchen Macher.
  careerCtaButtonText: Offene Stellen ansehen
features:
  - 24h Notdienst bei Sturmschäden
  - Reparatur aller Dacharten
  - Wartungsservice
  - Rinnenreinigung
benefits:
  - Schnelle Reaktionszeiten
  - Kostenlose Ersteinschätzung
  - Direkte Abrechnung mit Versicherung möglich
  - Werterhalt Ihrer Immobilie
sections:
  - title: Reparatur vor Austausch
    icon: ShieldCheck
    content: Wir verkaufen Ihnen kein neues Dach, wenn eine Reparatur reicht. Wir prüfen den Zustand ehrlich. Oft lassen sich Schäden lokal beheben, ohne gleich alles neu zu machen.
  - title: Wartungsservice
    icon: Calendar
    content: Einmal jährlich kommen wir vorbei - Rinnen reinigen, Sichtprüfung der Ziegel und Anschlüsse, Protokoll für Ihre Versicherung. Das gibt Sicherheit.
processSteps:
  - step: 1
    title: Meldung
    text: Sie rufen an oder schicken ein Foto des Schadens.
  - step: 2
    title: Besichtigung
    text: Wir schauen uns den Schaden vor Ort an (bei Gefahr im Verzug sichern wir sofort).
  - step: 3
    title: Reparatur
    text: Kleine Reparaturen erledigen wir oft sofort. Für Größeres erhalten Sie ein Festpreis-Angebot.
contactIds:
  - sabine
  - isabel
faq:
  - question: Was kostet eine Dachreparatur?
    answer: Anfahrtspauschale 25 € Kreis Viersen, 35 € außerhalb. Stundenverrechnungssätze 74,50 € Geselle, 67,50 € Helfer, 92,50 € Meister, 47,50 € Azubi.
  - question: Helfen Sie bei Sturmschäden?
    answer: Ja, wir sichern das Dach und dokumentieren den Schaden für Ihre Versicherung.
  - question: Reinigen Sie auch Dachrinnen?
    answer: Ja, das gehört zu unseren Standard-Serviceleistungen.
---

## Reparatur & Wartung

Das Dach ist das am stärksten beanspruchte Bauteil Ihres Hauses. Hitze, Frost, Sturm und Hagel setzen ihm zu. Mit unserem Reparatur- und Wartungsservice sorgen wir dafür, dass Ihr Dach dicht bleibt und kleine Probleme nicht zu großen Schäden werden.
